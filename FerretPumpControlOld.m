function varargout = FerretPumpControlOld(varargin)
% FERRETPUMPCONTROLOLD MATLAB code for FerretPumpControlOld.fig
%      FERRETPUMPCONTROLOLD, by itself, creates a new FERRETPUMPCONTROLOLD or raises the
%       existing singleton*.
%
%      H = FERRETPUMPCONTROLOLD returns the handle to a new FERRETPUMPCONTROLOLD or the
%       handle to the existing singleton*.
%
%      FERRETPUMPCONTROLOLD('CALLBACK',hObject,eventData,handles,...) calls the
%       local function named CALLBACK in FERRETPUMPCONTROLOLD.M with the given
%       input arguments.
%
%      FERRETPUMPCONTROLOLD('Property','Value',...) creates a new FERRETPUMPCONTROLOLD or 
%       raises the existing singleton*.  Starting from the left, property 
%       value pairs are applied to the GUI before FerretPumpControlOld_OpeningFcn 
%       gets called.  An unrecognized property name or invalid value makes 
%       property application stop.  All inputs are passed to 
%       FerretPumpControlOld_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 28-Apr-2014 13:34:59

%%%%%%%% Begin initialization code - DO NOT EDIT %%%%%%%%%%%%%%%%%%%%%%%%%
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @FerretPumpControlOld_OpeningFcn, ...
                   'gui_OutputFcn',  @FerretPumpControlOld_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% DRUG INFORMATION
c1.drug.name = 'IV Solution';
c1.drug.concentration = 1;
c1.drug.units = 'mL';
c1.drug.instructions = 'LRS + dextrose comes prepared.';
c1.drug.doseFactor = c1.subject.weight/c1.drug.concentration;
c1.drug.doseMax = 10;
c1.drug.doseStep = 0.25;
c1.drug.doseInitial = 2.5;
...
% --- Executes just before FerretPumpControlOld is made visible. ------------
function FerretPumpControlOld_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to FerretPumpControlOld (see VARARGIN)

% Choose default command line output for FerretPumpControlOld
...
function varargout = FerretPumpControlOld_OutputFcn(~, ~, handles) 
...
close FerretPumpControlOld;                                                 % REMOVE THE GUI
